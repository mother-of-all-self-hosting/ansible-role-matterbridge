...
#
# Tags look like `v<Matterbridge version>-<release>`:
#
# - if defaults/main.yml points at a Matterbridge version that has never been
#   released, the release counter restarts at 0 (`v1.26.0-0`)
# - otherwise the counter is incremented (`v1.26.0-1`), but only if something
...
version="$(sed -nE 's|^matterbridge_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Matterbridge version from $defaults_path"
	exit 1
fi

# This role's version values carry no leading `v` (e.g. `1.26.0`) while its
# tags do, but trimming one that is not there is harmless and keeps the tag
# correct should the value ever gain one.
